...
#ifndef SOLE_INCLUDE_H_
#define SOLE_INCLUDE_H_
...
//uuid uuid4(); // UUID v4, pros: anonymous, fast; con: uuids "can clash"s
...
inline std::string uuid::pretty() const {
  ...
  version == 4 && ss << "randbits=" << std::setw(8) << (ab & 0xFFFFFFFFFFFF0FFFULL) << std::setw(8)
                     << (cd & 0x3FFFFFFFFFFFFFFFULL) << ',';
...
$windows(
    ...
    inline int gettimeofday_local(struct timeval *tv, struct timezone *tz) {
      FILETIME ft;
      uint64_t tmpres = 0;
...
$lelse($belse($emselse( // if not linux, if not bsd, if not emscripten... valid for apple/win32
           inline int clock_gettime(int /*clk_id*/, struct timespec *t) {
             struct timeval now;
             int rv = gettimeofday_local(&now, NULL);
             if (rv) return rv;
             t->tv_sec = now.tv_sec;
...
inline bool get_any_mac(std::vector<unsigned char> &_node) {
  $windows({
             ...
             if (rc == ERROR_BUFFER_OVERFLOW) {
               ...
               return $no("cannot get network adapter list")
               false;
             }
             ...
             if (!found)
               return $no("no Ethernet adapter found")
             false;
  ...
  $bsd({
         ...
         if (rc) return $no("cannot get network adapter list")
         false;
         ...
         if (!foundAdapter) return $no("cannot determine MAC address (no suitable network adapter found)")
         false;
         return true;
       })
  ...
  $linux({
           struct ifaddrs *ifaphead;
           if (getifaddrs(&ifaphead) == -1) return $no("cannot get network adapter list")
           false;
           ...
           for (struct ifaddrs *ifap = ifaphead; ifap; ifap = ifap->ifa_next) {
             ...
             if (std::strcmp("lo", ifap->ifa_name) == 0) {
               close(s);
               continue;
             }  // loopback address is zero
  ...
  $unix({
          ...
          if (gethostname(name, sizeof(name)))
            return $no("cannot get host name")
          false;

          struct hostent *pHost = gethostbyname(name);
          if (!pHost) return $no("cannot get host IP address")
          false;

          int s = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
          if (s == -1) return $no("cannot open socket")
          false;
          ...
          close(s);
          if (rc < 0) return $no("cannot get MAC address")
          false;
